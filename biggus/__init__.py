...
class ArrayStack(Array):
    ...
    def __init__(self, stack):
        ...
        first_nan = np.isnan(fill_value)
        for array in stack.flat:
            if (array.shape != item_shape or array.dtype != dtype or
                    first_nan != np.isnan(array.fill_value) or
                    (not first_nan and array.fill_value != fill_value)):
...
class LinearMosaic(Array):
    def __init__(self, tiles, axis):
        ...
        common_nan = np.isnan(common_fill_value)
        ...
        for tile in tiles[1:]:
            ...
            if (common_nan != np.isnan(tile.fill_value) or
                    (not common_nan and tile.fill_value != common_fill_value)):
